...
const EditMemberModal = ({ open, onClose, onSuccess, memberId }: EditMemberModalProps) => {
  ...
  const fetchMember = useCallback(async () => {
    ...
    try {
      ...
      if (memberData.branchId) {
        fetchDepartments(memberData.branchId);
      }

      if (memberData.departmentIds.length > 0) {
        memberData.departmentIds.forEach((deptId) => fetchUnits(deptId));
      }
    } catch (error: any) {
  ...
  const handleChange = (
    ...
    if (name) {
      ...
      if (name === 'branchId' && value) {
        ...
        setFormData((prev) => ({ ...prev, departmentIds: [], unitIds: [] }));
      }
    }
  };
  ...
  const handleDepartmentChange = (event: SelectChangeEvent<string[]>) => {
    ...
    setFormData((prev) => ({
      ...
      unitIds: prev.unitIds.filter((unitId) =>
        selectedIds.some((deptId) => departmentUnits[deptId]?.some((unit) => unit.id === unitId))
      ),
    }));
    setDepartmentUnits((prev) => {
      const newUnits = { ...prev };
      Object.keys(newUnits).forEach((deptId) => {
        if (!selectedIds.includes(deptId)) {
          delete newUnits[deptId];
        }
      });
      return newUnits;
    });
    setHasFetchedUnits((prev) => {
      const newFetched = { ...prev };
      Object.keys(newFetched).forEach((deptId) => {
        if (!selectedIds.includes(deptId)) {
          delete newFetched[deptId];
        }
      });
      return newFetched;
    });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Validate department and unit assignments
      if (formData.departmentIds.length > 0 && formData.unitIds.length > 0) {
        const invalidUnits = formData.unitIds.filter((unitId) =>
          !formData.departmentIds.some((deptId) =>
            departmentUnits[deptId]?.some((unit) => unit.id === unitId)
          )
        );
        if (invalidUnits.length > 0) {
          throw new Error("Selected units must belong to the chosen departments");
        }
      }
