...
const Form = () => {
  ...
  const handleEditSave = async () => {
    ...
    setEditLoading(true); // start edit loader
    try {
      const payload: any = {
      ...
      // Only include options if type === "multi-choice"
      ...
      await Api.patch(
        `/follow/edit-question/${selectedQuestion.id}?branchId=${branchId}`,
        payload
      );

      showPageToast("Question updated successfully!", 'success');
      // Optional: Refresh question list
      ...
      setEditLoading(false); // stop edit loader
    }
  };

  const updateQuestion = (key: keyof Question, value: any) => {
    setSelectedQuestion((prev: any) => (prev ? { ...prev, [key]: value } : prev));
  };
  ...
  const removeOption = (index: number) => {
    setSelectedQuestion((prev: any) =>
      prev
        ? { ...prev, options: prev.options?.filter((_: any ,i: any) => i !== index) || [] }
        : prev
    );
  };
  ...
  const updateOption = (index: number, value: string) => {
    setSelectedQuestion((prev: any) =>
      prev
        ? {
            ...prev,
            options: prev.options?.map((opt: any, i: any) => (i === index ? value : opt)) || [],
          }
        : prev
    );
  };

  // Compute visibleFields using useMemo to update automatically when checkedFields changes
  ...
  // Fetch questions from API and initialize checkedQuestions as unchecked
  const fetchQuestions = async () => {
    try {
      ...
      // Initialize checkedQuestions with all questions unchecked
  ...
  const handleSaveForm = async () => {
    ...
    if (checkedFields.fullname === false && checkedFields.phoneNumber === false) {
      showPageToast(" Full Name or Phone Number must be selected", 'error');
      return;
    }

    if (!checkedFields.fullname && !checkedFields.phoneNumber && !Object.values(checkedQuestions).some((v) => v)) {
    ...
    try {
      ...
      showPageToast("Form saved successfully", 'success'); 
        ...
        // Reset checkedQuestions to unchecked for all questions
        setCheckedQuestions(questions.reduce((acc: Record<string, boolean>, q: any) => {
          acc[q.id] = false;
          return acc;
        }, {}));   
        }, 1000)
    ...
    <DashboardManager>
      <Box sx={{ py: 4, px: { xs: 2, sm: 3 }, minHeight: "100%" }}>
              ...
              <TextField
                ...
                InputProps={{
                    ...
                    "& .MuiInputBase-input.Mui-disabled": {
                        color: "#4B4B4B", // muted input text when disabled
                    ...
                    "&.Mui-disabled .MuiOutlinedInput-notchedOutline": {
                        borderColor: "#4B4B4B", // muted border when disabled
                    },
                    "& input::placeholder": {
                ...
                InputLabelProps={{
                    ...
                    "&.Mui-disabled": {
                        color: "#4B4B4B", // muted label when disabled
                    },
                    },
                }}
              />
                ...
                renderInput={(params) => (
                  <TextField
                    ...
                    sx={{
                      ...
                      "& .MuiFormLabel-root": {
                        color: checkedFields.dateOfBirth ? "#4CAF50 !important" : "#4B4B4 !importantB",
                      },  
                      "& .MuiAutocomplete-popupIndicator": {  color: checkedFields.dateOfBirth ? "#4CAF50" : "#4B4B4B !important" },                    
                    }}
                  />
                )}
              />
            ...
            <Box sx={{ position: "relative" }}>
              <FormControl fullWidth disabled={!checkedFields.attendanceDuration}>
                <InputLabel    sx={{
                    ...
                    "&.Mui-focused": { color: "#4CAF50 !important" },
                    "&.Mui-disabled": { color: "#4B4B4B !important" },
                  }}>Attendance Duration</InputLabel>
                <Select
        ...
        <Box sx={{ display: "flex", justifyContent: "flex-end", mt: 3 }}>
          ...
          >
            {formLoading ?  <span className="text-gray-400">Creating...</span> : "Create Form"}
          </Button>
        </Box>

        {/* ✳️ EDIT DIALOG USING YOUR PROVIDED UI STYLE */}
